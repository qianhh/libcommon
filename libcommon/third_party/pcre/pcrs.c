...
/*********************************************************************
 *
 * Function    :  pcrs_subsitute_command
 *
 ...
 *
 *********************************************************************/
char *pcrs_subsitute_command(const char *subject, const char *pcrs_command, int *hits)
{
   size_t size;
   char *result = NULL;
   pcrs_job *job;
 ...
 *          3  :  hits = int* for returning  the number of modifications
 *
 * Returns     :  count of matched text
 *
 *********************************************************************/
int pcrs_match_command(const char *subject, const char *pcre_command,const char* pcre_option,char **result, size_t *result_position)
{
   size_t size;
   int flags;
   int hits = 0;
   ...
   assert(pcre_command);
   assert(result);
   assert(result_position);

   options = pcrs_parse_perl_options(pcre_option, &flags); 
   /*
    * Compile the pattern
    */
   pattern = pcre_compile(pcre_command,options, &error,&errptr, NULL);
   if (pattern == NULL)
   {
      return errptr;
   }

   hits = pcre_exec(pattern, NULL, subject, strlen(subject), 0, 0, ovector, 10000); 
...
pcrs_job *pcrs_compile_dynamic_command(char *pcrs_command, const struct pcrs_variable v[], int *error)
{
   ...
   /*
   * FIXME:2013-04-23 arron 
   */
   while ((NULL != v->name) && (NULL != pcrs_command) && (NULL != v->value))
 //  while ((NULL != v->name) && (NULL != pcrs_command))
   {
      ...
      pcrs_command_tmp = pcrs_subsitute_command(pcrs_command, buf, error);
